...
def get_active_task_state_or_404(identifier: str) -> TaskState:
    from .models import TaskState

    return get_object_or_404(TaskState.objects.active(), identifier=identifier)
...
def get_claim_count_cache_key(job: Job) -> str:
    return f"job_dequeue_count_{job.id}"
